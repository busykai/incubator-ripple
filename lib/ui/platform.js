...
//        tooltip = ripple('ui/plugins/tooltip');
    ...
    function _updatePlatformDeviceSelect(platformID, currentDeviceKey) {
        var devicesSelect = $("#device-select");

        function doUpdatePlatformDeviceSelect(listOfSortedDevices) {

            devicesSelect.innerHTML = "";

            _.each(listOfSortedDevices, function (dev) {
                var deviceNode = $("<option/>", {
                    "text": dev.name,
                    "value": dev.id
                });

                devicesSelect.append(deviceNode);
            });
            devicesSelect.val(currentDeviceKey);
        }

        peer.getDevicesForPlatform(platformID, function (list) {
             doUpdatePlatformDeviceSelect(list
                                            .sort(function (a, b) {
...
//                    $("#" + constants.COMMON.DEVICE_SELECT_ID).effect("highlight", {color: "#62B4C8"}, 500, function () {
//                        _animateChangePlatformButton();
//                    });
                });


                $("#change-platform").bind("click", changePlatformOrDevice);
                $("#device-select").bind("change", changePlatformOrDevice);
                ...
                _updatePlatformDeviceSelect(currentPlatformID, currentDeviceKey);
