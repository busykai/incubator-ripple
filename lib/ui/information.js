...
define(["jquery", "lodash"], function ($, _) {
    ...
    function _updateInformationView() {
        ...
        var infoPane = document.getElementById("information-sub-container"),
            infoList = [],
            count = 0,
            device,
            platform,
        ...
        //    }
            infoList.push("<section><label class=\"ui-text-label\">Platform: </label><span class=\"text-is-selectable\">" + platform.name + "</span></section>");
            ...
            if (device.notes) {
                _.each(device.notes, function (note) {
        ...
        peer.getCurrentPlatform(function (cPlatform) {
            platform = cPlatform;
            count++;
            proceedIfDone();
        });

        peer.getCurrentDevice(function (cDevice) {
            device = cDevice;
            count++;
            proceedIfDone();
        });
    ...
    function _updateBanner(icon, count) {
        ...
        if (icon && (count === undefined || count !== 0)) {
            count = count || "";
            $("#information-sub-container").show();
            $(bannerSection).fadeToggle(1000);
            $(bannerSection).fadeIn(1000);
        }
        else {
            $(bannerSection).fadeOut(1000);
        }

        iconImg.src = window.location.origin + "/" + icon;
    ...
    return {
        ...
        initialize: function () {
            peer.on("BannerUpdated", function (icon, count) {
                _updateBanner(icon, count);
            });

            peer.on("PlatformChangedEvent", function () {
                _updateInformationView();
            });

            peer.on("WidgetInformationUpdated", function () {
                _updateInformationView();
            });
