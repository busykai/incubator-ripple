...
function localInjection() {
    ...
    function handle(req, res, next) {
        ...
        if (req.query.enableripple) {
            fs.readFile(path.join(HOSTED_PKG_DIR, req.path.match(/\/$/) ? "index.html" : req.path), "utf-8", function (err, data) {
                if (err) {
                    throw new Error(err);
                }
                data = data.replace(HEAD_TAG, '<head><script>var wsRPCPort=' + req.wsRPCPort + ';</script>');
                res.send(data);
            });
